...
def transfer(facenet_output, features, age_labels, gender_labels, training=True, weight_decay=5e-4):
	# this fc layer replaces the bottleneck layer in facenet
  net = slim.fully_connected(facenet_output, 512,
  														scope='Net/Bottleneck', reuse=False)
  ...
  branch2 = slim.fully_connected(net, 768, scope='Net/Branch2', reuse=False)
  branch2 = slim.dropout(branch2, 0.8, is_training=training, scope='Dropout')
  gender_logits = slim.fully_connected(branch2, 2,
                                       weights_initializer=slim.initializers.xavier_initializer(),
                                       weights_regularizer=slim.l2_regularizer(weight_decay),
